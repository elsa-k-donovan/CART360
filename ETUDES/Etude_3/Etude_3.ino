 ...
 * if it is being pressed then change the mode variable.
 * REMEMBER:
 * mode == 0 is reset 
 * mode == 1 is live
 * mode == 2 is record
...
void live()
{
  ...
  //NO STORAGE SHOULD BE HAPPENING HERE
    int val = analogRead(NOTE_IN_PIN);

  if(val > 0){
      
      //NOTE_IN_PIN needs to be read in an analog way so that it can create a different sound
...
void record()
{
  ...
  //analog read the music buttons and their resistance, assign it to a variable
  int val;
  val = analogRead(NOTE_IN_PIN);
  ...
  Serial.println(val);
   ...
   if(val > 0){ 
          //play the note to the speaker
          tone(BUZZER_PIN, val);
        
          delay(duration);
          
          //make sure there are not more notes that are stored than MAX_NOTES amount
          if (countNotes < MAX_NOTES){
        
              //fill up the array with the pressed notes
              notes[countNotes] = val;
        
              //increase countNotes by 1
              countNotes++;
             }
   }

  //trying to reduce static noise by stopping buzzer after every notes
   noTone(BUZZER_PIN);
...
{
  //IMPLEMENT
  //randomly select order, create new array, play through this new array

  //declare new random sequence array
  int randomSequence[countNotes];

   //set a new array
   for (int i = 0; i < countNotes; i++){
      
      randomSequence[i] = random(0, (countNotes+1));
      
   }

Serial.println("Buttons being played: ");

   //play the new array
   for (int i = 0; i < countNotes; i++){
      
        tone(BUZZER_PIN, randomSequence[i]);
